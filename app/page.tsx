...
export default function Home() {
  ...
  const filtered = useMemo(() => docs.filter(d => (d.assunto + d.id + d.origem).toLowerCase().includes(query.toLowerCase())), [query]);
    ...
    <div className="app-shell">
      <aside className="sidebar">
        <button className="brand brand-button" onClick={() => setView("portal")} aria-label="Voltar ao portal">
          ...
          <span><b>SIGED</b><small>Expediente Digital</small></span>
        </button>
        <nav>
          <p>VISÃO GERAL</p>
      ...
      {newOpen && <NewDocument close={() => setNewOpen(false)} save={() => { setNewOpen(false); notify("Expediente MTGAS/ENT/2026/0843 registado com sucesso"); }} />}
    </div>
  );
}
  ...
  return <main className="portal">
    ...
    <section className="hero">
      ...
      <div className="hero-copy">
        <span className="eyebrow"><i /> TRANSFORMAÇÃO DIGITAL DO MTGAS</span>
...
function NewDocument({close,save}:{close:()=>void;save:()=>void}){return <div className="modal-backdrop" onMouseDown={close}><form className="modal" onMouseDown={e=>e.stopPropagation()} onSubmit={e=>{e.preventDefault();save()}}><div className="modal-head"><div><span>NOVO REGISTO</span><h2>Registar expediente</h2><p>Introduza os dados e encaminhe para a unidade responsável.</p></div><button type="button" onClick={close}>×</button></div><div className="form-grid"><label>Tipo de expediente<select required><option>Documento recebido</option><option>Documento interno</option><option>Documento de saída</option></select></label><label>Tipo documental<select required><option>Ofício</option><option>Nota</option><option>Relatório</option><option>Informação</option><option>Requerimento</option></select></label><label className="wide">Assunto<input required placeholder="Assunto principal do documento"/></label><label>Entidade remetente<input required placeholder="Instituição ou unidade de origem"/></label><label>Unidade responsável<select required>{units.map(u=><option key={u}>{u}</option>)}</select></label><label>Prioridade<select><option>Normal</option><option>Alta</option><option>Urgente</option></select></label><label>Prazo de resposta<input type="date" required/></label><label className="wide">Observações<textarea placeholder="Resumo ou instruções de tramitação"/></label><label className="wide upload">⇧ <b>Carregar documento e anexos</b><small>PDF, DOCX, XLSX ou imagem • máximo 20 MB</small><input type="file" multiple/></label></div><div className="modal-actions"><button type="button" className="secondary" onClick={close}>Cancelar</button><button type="submit" className="primary">Registar e encaminhar <Icon name="arrow"/></button></div></form></div>}
